...
class Program(tk.Frame, object):
	...
	def addEvent(self):
		...
		self.dayC = ttk.Combobox(window, state = 'readonly', textvariable = self.dayVar)
		...
		self.monthC = ttk.Combobox(window, state = 'readonly', textvariable = self.monthVar)
		...
		self.yearC = ttk.Combobox(window, state = 'readonly', textvariable = self.yearVar)
		...
		self.priorityC = ttk.Combobox(window, state = 'readonly', textvariable = self.priorityVar)
	...
	def insert(self):
		date = self.yearC.get()+'-'+self.length2(self.monthC.get())+'-'+self.length2(self.dayC.get())
		self.events.add(date, self.nameE.get(), self.categoryE.get(), self.priorityC.get())
